...
# Usage: python3 run-all-mutants.py {EPA|SBES} {subject_name} {gen_strategy} {mrs_to_fuzz} {allow_epa_loops}
...
for seed in seeds:
    for mutant_number in mutant_numbers:
        print(f'Running over mutant {mutant_number} for: {subject_name} with seed:{seed}')
        result = subprocess.run(f'experiments/run-over-mutant.sh {subject_set} {subject_name} {gen_strategy} {mrs_to_fuzz} {allow_epa_loops} {seed} {mutant_number}', shell=True, stdout=subprocess.PIPE)
